...
3. Now we are at the quiz, it has to detect if we've seen this question before by reading `answers.lzma` in `AppData/Local/tassomai-automation`
...
5. Otherwise, if the question isn't found in `answers.lzma` then it will call a function that answers a random box by listing all the boxes and using the `random` libary to choose one in the list to click it and return if it's correct or not.
...
8. When the program has been exited, it will grab the dictionary in `answers.lzma` and store it online in a private repository that is not viewable by any third party due to in violation of Tassomai TOS.

**Currently as of 24/02/21, 3896+ questions have been answered!**
